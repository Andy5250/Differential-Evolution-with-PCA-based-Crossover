...
from sklearn.decomposition import PCA, IncrementalPCA, RandomizedPCA, SparsePCA
...
#    self.population = np.fft.fft(self.population)
    ...
    while not converged:
      ...
      population_sorted1 = np.array(sorted(zip(self.scores, self.population), key=lambda x: x[0]))
      self.population_sorted = []
      for scores, population in population_sorted1:
          self.population_sorted.append(population)
      if self.count%1==0:
          self.pca.fit(self.population_sorted[0 : (self.evaluator.n+5)])  #不能低于30 否则无法产生30维pca
...
#    self.CRL = [self.cr]*self.population_size
  ...
  def score_population(self):
    for vector,ii in zip(self.population,xrange(self.population_size)):
      tmpVector = np.zeros(self.evaluator.n*5)
      tmpVector[self.Round*self.evaluator.n : (self.Round+1)*self.evaluator.n] = vector
      tmp_score = self.evaluator.target(tmpVector)
...
#      use_f = np.random.random()/1.2 + 0.2/1.2
...
#      vi = np.add(x1 , np.multiply(use_f,np.dot(self.transMatl[i1], np.subtract(x2,x3))))
      ...
      tmpVector = np.zeros(self.evaluator.n*5)
      tmpVector[self.Round*self.evaluator.n : (self.Round+1)*self.evaluator.n] = test_vector
      test_score = self.evaluator.target( tmpVector )  #产生过程中的随进向量库 
