...
#    self.table = self.data.add_sheet('pca', cell_overwrite_ok=True) 
    self.fileHandle = open('D:/Users/zdz/cec15_100/dataCC%d.txt'%index, 'w')     
    self.x = None
    self.n = dim
    self.func = func
    self.dim = dim
    ...
    self.optimizer = data_DE.differential_evolution_optimizer(
                        self,population_size=self.n*10,n_cross=0,cr=0.5, max_iter=1000000,
                         eps=1e-8, monitor_cycle=200000, show_progress=True, grouplist = grouplist)
